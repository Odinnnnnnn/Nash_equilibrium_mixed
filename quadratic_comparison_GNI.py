...
class Quadratic(object):
    ...
    def game_setting(self, game_matrix_1, game_matrix_2, game_vector_1, game_vector_2):
        if self.game_if_set == 0:
            ...
            self.game_vector_1 = tf.convert_to_tensor(game_vector_1, dtype = tf.float32)
            self.game_vector_2 = tf.convert_to_tensor(game_vector_2, dtype = tf.float32)
            self.game_if_set = 1
    ...
    def build_network_pure(self):
        ...
        player_action_pure = tf.expand_dims(tf.concat([self.player1_action_pure, self.player2_action_pure], axis = 0), axis = 0)

        self.expected_utility1_pure = tf.reduce_mean(tf.matmul(tf.matmul(player_action_pure, self.game_matrix_1),
                                                                  tf.transpose(player_action_pure, perm = [1,0]))) + tf.reduce_mean(tf.matmul(player_action_pure, self.game_vector_1))
        self.expected_utility2_pure = tf.reduce_mean(tf.matmul(tf.matmul(player_action_pure, self.game_matrix_2),
                                                                  tf.transpose(player_action_pure, perm = [1,0]))) + tf.reduce_mean(tf.matmul(player_action_pure, self.game_vector_2))
        ...
        player_action_pure_1 = tf.expand_dims(tf.concat([self.player1_action_pure_local, self.player2_action_pure],axis = 0), axis=0)
        player_action_pure_2 = tf.expand_dims(tf.concat([self.player1_action_pure, self.player2_action_pure_local], axis = 0), axis=0)

        self.local_expected_utility1_pure = tf.reduce_mean(tf.matmul(tf.matmul(player_action_pure_1, self.game_matrix_1),
                                                                  tf.transpose(player_action_pure_1, perm = [1,0]))) + tf.reduce_mean(tf.matmul(player_action_pure_1, self.game_vector_1))
        self.local_expected_utility2_pure = tf.reduce_mean(tf.matmul(tf.matmul(player_action_pure_2, self.game_matrix_2),
                                                                  tf.transpose(player_action_pure_2, perm = [1,0]))) + tf.reduce_mean(tf.matmul(player_action_pure_2, self.game_vector_2))
    ...
    def build_model(self):
        ...
        player_action_batch = tf.concat([player1_action_batch, player2_action_batch], axis = 1)

        self.expected_utility1 = tf.reduce_mean(tf.diag_part(tf.matmul(tf.matmul(player_action_batch, self.game_matrix_1),
                                                                  tf.transpose(player_action_batch, perm = [1,0])))) + tf.reduce_mean(tf.matmul(player_action_batch, self.game_vector_1))
        self.expected_utility2 = tf.reduce_mean(tf.diag_part(tf.matmul(tf.matmul(player_action_batch, self.game_matrix_2),
                                                                  tf.transpose(player_action_batch, perm = [1,0])))) + tf.reduce_mean(tf.matmul(player_action_batch, self.game_vector_2))
        ...
        local_player1_batch = tf.concat([self.player1_action_local, player2_action_batch], axis = 1)
        local_player2_batch = tf.concat([player1_action_batch, self.player2_action_local], axis = 1)

        self.local_expected_utility1 = tf.reduce_mean(tf.diag_part(tf.matmul(tf.matmul(local_player1_batch, self.game_matrix_1),
                                                                  tf.transpose(local_player1_batch, perm = [1,0])))) + tf.reduce_mean(tf.matmul(local_player1_batch, self.game_vector_1))
        self.local_expected_utility2 = tf.reduce_mean(tf.diag_part(tf.matmul(tf.matmul(local_player2_batch, self.game_matrix_2),
                                                                  tf.transpose(local_player2_batch, perm = [1,0])))) + tf.reduce_mean(tf.matmul(local_player2_batch, self.game_vector_2))
    ...
    def optimize_model_pure(self):
        self.file_out = open('quadratic_comparison_5.txt', 'w')
...
def main():
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        game_matrix_1 = np.random.rand(2 * game_dimension, 2 * game_dimension)
        game_matrix_2 = np.random.rand(2 * game_dimension, 2 * game_dimension)
        game_vector_1 = np.random.randn(2 * game_dimension, 1)
        game_vector_2 = np.random.randn(2 * game_dimension, 1)

        quadratic.game_setting(game_matrix_1, game_matrix_2, game_vector_1, game_vector_2)
