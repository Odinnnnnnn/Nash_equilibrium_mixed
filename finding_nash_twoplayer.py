...
class BiMatrix(object):
    def __init__(self, sess):
        ...
        self.random_uniform_1 = tf.placeholder(shape = [self.batch_size, self.sdim], dtype = tf.float32)
        self.random_uniform_2 = tf.placeholder(shape = [self.batch_size, self.sdim], dtype = tf.float32)
        self.random_uniform_3 = tf.placeholder(shape=[self.batch_size, self.sdim], dtype=tf.float32)
        self.random_uniform_4 = tf.placeholder(shape=[self.batch_size, self.sdim], dtype=tf.float32)
    ...
    def player_network(self, scope):
        if scope == 'player_number_1':
            random_sample = self.random_uniform_1
        else:
            random_sample = self.random_uniform_2
        ...
        output_layer = tf.nn.tanh(tf.matmul(hidden_layer_6, self.W7) + tf.tile(tf.expand_dims(self.b7, axis = 0), [self.batch_size, 1]))
    ...
    def player_network_local(self, scope):
        if scope == 'player_number_1':
            ...
            random_sample = self.random_uniform_3
        else:
            L = self.expected_utility2
            random_sample = self.random_uniform_4
    ...
    def optimize_model(self):
        file_out_min = open('output_bimatrix_3_1.txt','w')
        ...
        for epoch in range(self.epoch_num):
            A = np.random.random((self.batch_size, self.sdim)).astype(np.float32)
            B = np.random.random((self.batch_size, self.sdim)).astype(np.float32)
            C = np.random.random((self.batch_size, self.sdim)).astype(np.float32)
            D = np.random.random((self.batch_size, self.sdim)).astype(np.float32)
            # print(self.sess.run(self.player1_action_local, feed_dict = {self.Lambda:Lambda, self.random_uniform_1:A, self.random_uniform_2: B}))
            self.sess.run(self.minimize, feed_dict = {self.Lambda:Lambda, self.random_uniform_1:A, self.random_uniform_2: B, self.random_uniform_3:C, self.random_uniform_4:D})
            loss = self.sess.run(self.loss_function, feed_dict = {self.Lambda: Lambda, self.random_uniform_1: A, self.random_uniform_2:B, self.random_uniform_3:C, self.random_uniform_4:D})
